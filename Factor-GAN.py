...
import numpy as np
from tensorflow.keras.layers import LSTM, Bidirectional, Dense, Flatten, Conv1D, BatchNormalization, LeakyReLU, Dropout
...
def make_generator_model(input_dim, output_dim, feature_size) -> tf.keras.models.Model:
    model = Sequential()
    model.add(LSTM(500, return_sequences=True, input_shape=(input_dim, feature_size)))
...
def make_discriminator_model():
    ...
    cnn_net.add(Conv1D(32, input_shape=(70, 1), kernel_size=5, strides=2, padding='same', activation=LeakyReLU(alpha=0.001)))
    cnn_net.add(Conv1D(64, kernel_size=5, strides=2, padding='same', activation=LeakyReLU(alpha=0.001)))
    cnn_net.add(Conv1D(128, kernel_size=5, strides=2, padding='same', activation=LeakyReLU(alpha=0.001)))
    cnn_net.add(Flatten())
